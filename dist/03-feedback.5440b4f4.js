...
reft.form.addEventListener("input", (0, _lodashThrottleDefault.default)(onFormInput, 500));
loadPage();
function onFormInput(e) {
    const { name , value  } = e.target;
    let savedData = (0, _storageDefault.default).load(STORAGE_KEY);
    savedData = savedData ? savedData : {};
    savedData[name] = value;
    (0, _storageDefault.default).save(STORAGE_KEY, savedData);
}
function loadPage() {
    const savedData = (0, _storageDefault.default).load(STORAGE_KEY);
    if (savedData) Object.entries(savedData).forEach(([name, value])=>{
...
function onFormSabmit(e) {
    ...
    if (email.value === "" || message.value === "") return console.log("Please fill in all the fields!");
    const formData = {
        name: email.value,
        message: message.value
    };
    console.log(formData);
    (0, _storageDefault.default).remove(STORAGE_KEY);
